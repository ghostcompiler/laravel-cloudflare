...
class StreamManager extends AbstractManager
{
    ...
    public function all(string $accountId)
    {
        ...
        return $this->hydrate($response, function (array $data) {
            $items = array_map(fn (array $item) => \Vendor\Cloudflare\DTOs\StreamVideo::fromArray($item), $data["result"] ?? []);
            return new \Vendor\Cloudflare\Collections\StreamVideoCollection($items);
        });
    }
    ...
    public function find(string $accountId, string $uid)
    {
        ...
        return $this->hydrate($response, function (array $data) {
            return \Vendor\Cloudflare\DTOs\StreamVideo::fromArray($data["result"] ?? []);
        });
    }
