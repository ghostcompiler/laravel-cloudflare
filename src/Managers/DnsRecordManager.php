...
class DnsRecordManager extends AbstractManager
{
    ...
    public function all(string $zoneId)
    {
        ...
        return $this->hydrate($response, function (array $data) {
            $items = array_map(fn (array $item) => \Vendor\Cloudflare\DTOs\DnsRecord::fromArray($item), $data["result"] ?? []);
            return new \Vendor\Cloudflare\Collections\DnsRecordCollection($items);
        });
    }
    ...
    public function paginate(string $zoneId, int $perPage = 25, int $page = 1)
    {
        ...
        return $this->hydrate($response, function (array $data) {
            ...
            return new \Vendor\Cloudflare\Responses\PaginatedResponse(new \Vendor\Cloudflare\Collections\DnsRecordCollection($items), $meta);
        });
    }
    ...
    public function find(string $zoneId, string $id)
    {
        ...
        return $this->hydrate($response, function (array $data) {
            return \Vendor\Cloudflare\DTOs\DnsRecord::fromArray($data["result"] ?? []);
        });
    }
