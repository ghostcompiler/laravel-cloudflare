...
class KvNamespaceManager extends AbstractManager
{
    ...
    public function all(string $accountId)
    {
        ...
        return $this->hydrate($response, function (array $data) {
            $items = array_map(fn (array $item) => \Vendor\Cloudflare\DTOs\KvNamespace::fromArray($item), $data["result"] ?? []);
            return new \Vendor\Cloudflare\Collections\KvNamespaceCollection($items);
        });
    }
    ...
    public function paginate(string $accountId, int $perPage = 25, int $page = 1)
    {
        ...
        return $this->hydrate($response, function (array $data) {
            ...
            return new \Vendor\Cloudflare\Responses\PaginatedResponse(new \Vendor\Cloudflare\Collections\KvNamespaceCollection($items), $meta);
        });
    }
    ...
    public function create(string $accountId, array $data)
    {
        ...
        return $this->hydrate($response, function (array $data) {
            return \Vendor\Cloudflare\DTOs\KvNamespace::fromArray($data["result"] ?? []);
        });
    }
